    ...
    if (new_scan_returned & !new_scan_srv.response.empty)
    {
        ...
        if (kf_select_returned)
        {
            ...
            if (kf__scan_returned & !kf_scan_srv.response.empty)
            {
                ...
                pcl::PointCloud<pcl::PointXYZ>::Ptr new_pointcloud = pointcloud_format_correction(
                        new_scan_srv.response.pointcloud);
                //### Receiving of keyframe scan converted into a pointcloud ###
                pcl::PointCloud<pcl::PointXYZ>::Ptr kf_pointcloud = pointcloud_format_correction(
                        kf_scan_srv.response.pointcloud);
                ...
                if (converged)
                {
                    ...
                    {
                        ROS_INFO("[scan_matching][SUCCESS] Converged fitness above threshold with %f.",
                                 converged_fitness);
                    ...
                    {
                        ROS_ERROR("[scan_matching][FAILED] Converged fitness below threshold with %f.",
                                  converged_fitness);
