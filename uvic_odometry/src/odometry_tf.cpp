...
void odometry_callback(const geometry_msgs::PoseStamped& input) {
  geometry_msgs::TransformStamped odom_trans;
  tf::TransformBroadcaster odom_broadcaster;
    
  odom_trans.header.stamp = input.header.stamp;
  odom_trans.header.frame_id = input.header.frame_id;
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber global_pose_sub = n.subscribe("odometry/global_pose", 50, odometry_callback);
